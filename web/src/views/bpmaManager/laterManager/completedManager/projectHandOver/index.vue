<template>
  <basic-split-pane class="projectHandOver-container"
                    :default-percent='75'
                    :min-percent='25'>
      ...
      <div class="left-container">
        <basic-handler>
          <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                     @click="addClick('add')">新增项目移交</el-button>
        ...
        <basic-table ref="refProjectHandOverTableData"
                     :dataTable="projectHandOverTableData"
                     :columns="columns"
                     @current-change="handleCurrentChange"
                     :pagination="modelSearchFilter"
                     @pagination="getAllProjectHandOverData"
        ...
        >
         ...
         <template slot-scope="obj" slot="ifWholeDeliver">
            {{formatIsOrNot(obj.scope.row.ifWholeDeliver)}}
          </template>
        </basic-table>
      </div>
    </template>
    <template slot="rightContainer">
      <div class="right-container" >
        <page-select-filter :selectData="selectData" :data="projectHandOverData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
      </div>
    </template>
...
<script>
  ...
  import {getDictionaryDataByTypeFilter,getProjectList} from '@/api/common/dataDictionary'
    export default {
      ...
      activated(){
        this.getAllProjectHandOverData()
      },
      created() {
      ...
      methods:{
        contractChange(data) {
          ...
          if (!data) {
            ...
            return this.projectHandOverTableData.data = []
          }
        ...
        addClick(param) {
          ...
          if (param === 'add') {
            ...
            this.$router.push({name: 'projectHandOverDetail', params: {id: this.currentRow.id}})
          }
        },
        /**
         * 获取列表所有数据
         * */
        getAllProjectHandOverData () {
          // console.log('this.modelSearchFilter',this.modelSearchFilter)
          getLastDeliverDataList(this.modelSearchFilter).then( (res) => {
            if(res.status && res.status === 200){

              this.projectHandOverTableData.data = res.data.rows;
              this.projectHandOverTableData.total = res.data.total;
        ...
        getAllItem () {
          getProjectList({page: 1, limit: 10000}).then((res) =>{
            ...
            if(res.status && res.status === 200){
              ...
              this.$set(this.projectHandOverData,'data',res.data.rows)
              console.log('projectHandOverData',this.projectHandOverData.data)
            }else{
              this.$message({message: res.message,type: 'warning'})
            }
          }).catch(error => {
         ...
         */
        clickSearchFilter() {
          this.$refs.refBasicSearch.searchFilter(this.$refs.refProjectHandOverTableData, this.modelSearchFilter, () => {
            this.getAllProjectHandOverData();
          });
        },
        /**
