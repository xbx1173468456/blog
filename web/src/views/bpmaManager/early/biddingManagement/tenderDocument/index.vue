<template>
  <basic-split-pane class="tenderDocument-container "
                    :default-percent='75'
                    :min-percent='25'
  >
        ...
        <div class="left-container">
          ...
          <pagination  :total="total" :page.sync="modelSearchFilter.page" :limit.sync="modelSearchFilter.limit" @pagination="getAlltenderDocument"/>
        </div>
        </template>
        <template slot="rightContainer">
          <div class="right-container">
            <page-select-filter :selectData="selectData" :data="tenderDocumentData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
          </div>
        </template>
    ...
    methods: {
      ...
      addClick(param) {
        ...
        if (param === 'add') {
          ...
          this.$router.push({name: 'TenderDocumentDetail', params: {id: this.currentRow.id}})
        }
      },
      /**
       * 获取投标文件列表所有数据
      ...
      getAlltenderDocument () {
        ...
        this.$store.dispatch('GetAllTenderDocument',this.modelSearchFilter).then( (res) => {
          if(res.status && res.status === 200){

            this.tableData = res.data.rows;
            this.total = res.data.total;
            console.log('获取投标文件列表所有数据',res)
          }else{
       ...
       * */
      getAllItem () {
        this.$store.dispatch('GetAllTenderDocumentItem',{page: 1, limit: 10000}).then((res) =>{

          if(res.status && res.status === 200){
            console.log('获取招标项目所有数据',res)

            //this.tenderDocumentData.data = res.data.rows;
            this.$set(this.tenderDocumentData,'data',res.data.rows)
            console.log('tenderDocumentData',this.tenderDocumentData.data)
          }else{
            this.$message({message: res.message,type: 'warning'})
          }
        }).catch(error => {
       ...
       * */
      getStage () {
        this.$store.dispatch('GetDictionaryDataByTypeFilterTenderDocument','SYS_PRO_STAGE')
          .then( (res) => {
            if(res.status && res.status === 200){
