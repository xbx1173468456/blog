<template>
  <basic-split-pane class="biddingStrategy-container "
                    :default-percent='75'
                    :min-percent='25'
  >
        ...
        <div class="left-container">
          ...
          <div class="handler-container">
              <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                         ...
                         @click="addClick('add')">新增策略</el-button>
            ...
            <template>
              ...
              >
                ...
                <el-table-column
                  prop="date"
                  label="是否投标"
                  width="80" align="center" :formatter="formatIsTender">
                </el-table-column>
                <el-table-column
          ...
          <pagination  :total="total" :page.sync="modelSearchFilter.page" :limit.sync="modelSearchFilter.limit" @pagination="getAllbiddingStrategy"/>
        </div>
        </template>
        <template slot="rightContainer">
          <div class="right-container">
            <page-select-filter :selectData="selectData" :data="strategyData" v-model="selectValue"@change="strategyChange" @refreshData="getBiddingStrategyItem"></page-select-filter>
          </div>
        </template>
  ...
  export default {
    ...
    methods: {
      ...
      addClick(param) {
        ...
        if (param === 'add') {
          ...
          this.$router.push({
            ...
            this.$message({message:'当前无项目，无法新增策略!',type:'warning'})
          }
        } else {
          this.$router.push({name: 'BiddingStrategyDetail', params: {id: this.currentRow.id}})
        }
      },
      /**
       * 获取投标策略列表所有数据
      ...
      getAllbiddingStrategy () {
          this.$store.dispatch('GetAllBiddingStrategy',this.modelSearchFilter).then( (res) => {
            if(res.status && res.status ===200){

              this.tableData = res.data.rows;
              this.total = res.data.total;
              console.log('获取投标策略列表所有数据',res)
       ...
       * 获取投标策略项目所有数据
      * */
      getBiddingStrategyItem () {
        this.$store.dispatch('GetAllBiddingStrategyItem',{page: 1, limit: 10000}).then((res) =>{

          if(res.status && res.status ===200){

            //this.strategyData.data =  res.data.rows;
            this.$set(this.strategyData,'data',res.data.rows)
            console.log('投标策略项目',res)
          //  this.$message.success(res.message)
          }else{
            this.$message({message: res.message,type: 'warning'})
          }
       ...
       * */
      getStage () {
        this.$store.dispatch('GetDictionaryDataByTypeFilterBiddingStrategy','SYS_PRO_STAGE')
          .then( (res) => {
            if(res.status && res.status === 200){
