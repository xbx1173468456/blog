      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增变更单</el-button>
...
<script>
  ...
  import tableTitle from './../tableTitle.1.json'
  ...
  export default {
    ...
    async created() {
      ...
      const invoiceTypeSelectData = await getDictionaryDataByTypeFilter('invoiceType')
    ...
    methods: {
      ...
      getContractReviews() {
        ...
        getContractReviews(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增变更单！')
        this.dialogFormVisible = true
        let { formData } = this
        this.$store.dispatch("cacheId", '0')
        if (lock) {
          this.$router.push({
            path: `/bpmaManager/early/undertakeContract/change/id/0/detail`,
            query: this.proData
          })
        } else {
          this.$router.push({
            path: `/bpmaManager/early/undertakeContract/change/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
