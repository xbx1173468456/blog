      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增扣罚</el-button>
...
<script>
  ...
  import { getProjectList, getValuePunishMaster } from '@/api/bpmaManager/outputValue/cost/punishment.js'
  ...
  export default {
    ...
    methods: {
      ...
      getValuePunishMaster() {
        ...
        getValuePunishMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增扣罚！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/outputValue/cost/punishment/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
