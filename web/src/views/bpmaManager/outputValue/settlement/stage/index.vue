      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增阶段结算单</el-button>
...
<script>
  ...
  import { getProjectList, getValueStagePayMasters } from '@/api/bpmaManager/outputValue/settlement/stage'
  ...
  export default {
    ...
    methods: {
      ...
      getValueStagePayMasters() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getValueStagePayMasters(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/outputValue/settlement/stage/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'handleDate':
            return this.handleDate ? this.handleDate.split(' ')[0] : ''
          case 'ifPay':
