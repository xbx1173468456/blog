<template>
  <basic-container class="bpmaManagerOutputValueVisaRegistration-container-list ">
      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增签证</el-button>
...
<script>
  ...
  import { getProjectList, getValueVisaMasters } from '@/api/bpmaManager/outputValue/visa/registration'
  ...
  export default {
    ...
    methods: {
      ...
      getValueVisaMasters() {
        ...
        getValueVisaMasters(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增签证！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/outputValue/visa/registration/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'approvePrice':
            if (!row.approvePrice) return '0.00'
            return this.$utilsBasic.toThousands(row.approvePrice)
          case 'applyPrice':
            if (!row.applyPrice) return '0.00'
            return this.$utilsBasic.toThousands(row.applyPrice)
          case 'payPrice':
            if (!row.payPrice) return '0.00'
            return this.$utilsBasic.toThousands(row.payPrice)
          case 'handleDate':
            return row.handleDate ? row.handleDate.split(' ')[0] : ''
          case 'ifPay':
            return this.formatIsOrNot(row.ifPay)
          default:
            return cellValue
        }
      },
