      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增索赔</el-button>
...
<script>
  ...
  import { getProjectList, getValueClaimMasters } from '@/api/bpmaManager/outputValue/claimDemage/registration.js'
  ...
  export default {
    ...
    methods: {
      ...
      getValueClaimMasters() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getValueClaimMasters(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getValueClaimMasters()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增索赔！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/outputValue/claimDemage/registration/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
