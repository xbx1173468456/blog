...
<script>
  ...
  import { getProjectList, getFinanceFeeBudgetMasters } from '@/api/bpmaManager/finance/cost/budget.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinanceFeeBudgetMasters() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getFinanceFeeBudgetMasters(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getFinanceFeeBudgetMasters()
      },
      addClick(lock) {
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/finance/cost/budget/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
