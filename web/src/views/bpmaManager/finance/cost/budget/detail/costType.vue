  ...
  <div class="content-container">
    <div class="right-container">
      ...
      <el-table :height="286" :data="showData" @row-click="rowChange" border highlight-current-row stripe>
        ...
        <el-table-column align="center" class-name="spacing-set" label="预算金额">
          <template slot-scope="scope">
    ...
    <basic-dialog v-el-drag-dialog :visible.sync="visibleSync" title="引入费用-选择" >
      <dialog-cost-type @selectionChange="selectionChange" filterValid isShowChecked
                        v-if="visibleSync" :linkData="showData"></dialog-cost-type>
...
<script>
  ...
  import { deleteFinanceFeeBudgetDetail } from '@/api/bpmaManager/finance/cost/budget.js'
    ...
    methods: {
      deleteClick() {
        this.$confirm('亲，您是否确定删除当前数据，删除后数据不能恢复！', '提示', {
          ...
          if (this.row.id) {
            // deleteFinanceFeeBudgetDetail(this.row.id).then(res => {
            //   if (res.rel) {
