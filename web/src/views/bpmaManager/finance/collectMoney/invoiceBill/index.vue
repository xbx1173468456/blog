      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增发票</el-button>
...
<script>
  ...
  import { getProjectList, getFinanceIncomeBills } from '@/api/bpmaManager/finance/collectMoney/invoiceBill.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinanceIncomeBills() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getFinanceIncomeBills(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/finance/collectMoney/invoiceBill/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
