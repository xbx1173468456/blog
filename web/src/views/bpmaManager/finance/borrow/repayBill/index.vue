<template>
  <basic-container class="repayBill-container-list ">
    <div class="splitPane-container" ref="refSplitPane">
      <basic-split-pane :default-percent='75' :min-percent='25' split="vertical">
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增还款</el-button>
...
<script>
  ...
  import { getProjectList, getFinanceReturns } from '@/api/bpmaManager/finance/borrow/repayBill.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinanceReturns() {
        ...
        getFinanceReturns(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getFinanceReturns()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增还款！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/finance/borrow/repayBill/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'payPrice':
            if (!row.payPrice) return
            return row.payPrice ? this.$utilsBasic.toThousands(row.payPrice) : '0.00'
          case 'incomeAccount':
