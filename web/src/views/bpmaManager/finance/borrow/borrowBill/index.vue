<template>
  <basic-container class="borrowBill-container-list ">
      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增借款</el-button>
...
<script>
  ...
  import { getProjectList, getFinanceBorrows } from '@/api/bpmaManager/finance/borrow/borrowBill.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinanceBorrows() {
        ...
        getFinanceBorrows(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getFinanceBorrows()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增借款！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/finance/borrow/borrowBill/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
