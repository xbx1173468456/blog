      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增付款</el-button>
            ...
            <div class="table-row table-container">
              <el-table ref="refUserManager" :height="heightUserManager" :data=tableData @row-click="rowClick" border
...
<script>
  ...
  import { getProjectList, getFinancePay } from '@/api/bpmaManager/finance/payment/document.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinancePay() {
        ...
        getFinancePay(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增发票！')
        let { formData } = this
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/finance/payment/document/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'totalShouldPayPrice':
            return this.$utilsBasic.toThousands(row.totalShouldPayPrice)
          case 'totalBillPrice':
            return this.$utilsBasic.toThousands(row.totalBillPrice)
          case 'totalPayPrice':
            return this.$utilsBasic.toThousands(row.totalPayPrice)
          case 'totalUnpayPrice':
            return this.$utilsBasic.toThousands(row.totalUnpayPrice)
          case 'applyPrice':
            return this.$utilsBasic.toThousands(row.applyPrice)
          case 'payPrice':
            return this.$utilsBasic.toThousands(row.payPrice)
          case 'payAccount':
            return this.$utilsBasic.moeryCard(row.payAccount)
          case 'incomeAccount':
            return this.$utilsBasic.moeryCard(row.incomeAccount)
          default:
            return cellValue
        }
      },
