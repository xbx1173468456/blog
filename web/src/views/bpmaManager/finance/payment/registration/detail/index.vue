<template>
  <div :style="windowHeight" class="detail-container-list">
    <basic-handler>
      <el-button @click="initProjectFormData" icon="el-icon-plus" plain size="small" type="primary">新增应付</el-button>
    ...
    <el-form :model="formData" :rules="rules" class="form-container" ref="form">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <basic-form-item label="应付金额">
            <basic-input :disabled="disabled" :format="formatToThousands" :formatValue="formatToMoney"
              placeholder="请输入应付金额" v-model="formData.payPrice" price></basic-input>
          </basic-form-item>
        </el-col>
        <el-col :span="8">
    ...
    <basic-dialog v-el-drag-dialog :visible.sync="visibleSyncContract" title="合同-选择" >
      <dialog-sub-contract :oppositeEnterpriseId="formData.enterpriseId" :proId="formData.proId"
...
<script>
  ...
  import {
    deleteFinanceShouldPay,
    reviewFinanceShouldPay,
    saveFinanceShouldPay,
    postFinanceShouldPay,
    getFinanceShouldPayDetail
  } from '@/api/bpmaManager/finance/payment/registration.js'
  ...
  export default {
    ...
    activated() {
      this.$utilsBasic.loadAddAndDetailData(this, () => {
        ...
        if (this.$route.params.detail != 'detail') {
          await this.getFinanceShouldPayDetail(this.$route.params.detail)
        } else {
          this.initProjectFormData()
        }
    ...
    methods: {
      ...
      getFinanceShouldPayDetail(id) {
        return getFinanceShouldPayDetail(id).then(res => {
          if (res.data) {
      ...
      initFormData(lock) {
        ...
        if (lock) {
          this.initProjectFormData()
          this.$router.replace({
            path: `/bpmaManager/finance/payment/registration/id/0/detail`,
            query: proData
          })
        } else {
          this.$router.replace({
            path: `/bpmaManager/finance/payment/registration/id/0/${this.formData.id}`,
            query: proData
          })
        }
      },
      deleteData() {
        if (this.formData.id) {
          this.$confirm('亲，您是否确定删除当前数据，删除后数据不能恢复！', '提示', {
            ...
            deleteFinanceShouldPay(this.formData.id).then(res => {
              if (res.rel) {
      ...
      review() {
        this.$confirm(' 亲，您是否确定审核当前数据，审核后数据不能修改！', '提示', {
          ...
          reviewFinanceShouldPay(this.formData).then(res => {
            if (res.rel) {
              this.isUpdate = true
              this.formData = res.data
              this.formData.payDate = new Date(this.formData.payDate).getTime()
      ...
      save(lock) {
        return new Promise((reslove, reject) => {
          ...
          this.$refs.form.validate(async valid => {
            ...
            if (this.formData.id) {
              res = await postFinanceShouldPay(this.formData)
            } else {
              res = await saveFinanceShouldPay(this.formData)
            }
            if (!res.rel) return
            this.formData = res.data
