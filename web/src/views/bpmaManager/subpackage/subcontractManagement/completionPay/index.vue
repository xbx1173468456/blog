<template>
  <basic-split-pane class="bpmaManager-container-list completionPay-container"
                    :default-percent='75'
                    :min-percent='25'>
    <template  slot="leftContainer">
      <div class="left-container">
        ...
        >
          ...
          <template slot-scope="obj" slot="shouldPayPriced">
            {{formatToThousands(obj.scope.row.shouldPayPriced)}}
          </template>
          <template slot-scope="obj" slot="shouldPayDate">
            {{formatDate(obj.scope.row.shouldPayDate)}}
          </template>
          <template slot-scope="obj" slot="qualityAssuranceScale">
            {{formatToThousands(obj.scope.row.qualityAssuranceScale)}}
          </template>
          <template slot-scope="obj" slot="qualityAssurancePrice">
            {{formatToThousands(obj.scope.row.qualityAssurancePrice)}}
          </template>
          <template slot-scope="obj" slot="qualityAssuranceDate">
            {{formatDate(obj.scope.row.qualityAssuranceDate)}}
          </template>
    ...
    <template slot="rightContainer">
      <div class="right-container" >
        <page-select-filter :selectData="selectData" :data="completionPayData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
      </div>
    </template>
      ...
      methods:{
        ...
        addClick(param) {
          ...
          if (param === 'add') {
            ...
            this.$router.push({name: 'CompletionPayDetail', params: {id: this.currentRow.id}})
          }
        },
        /**
         * 获取完工结算列表所有数据
        ...
        getAllCompletionPayData () {
          ...
          getAllCompletionPay(this.modelSearchFilter).then( (res) => {
         ...
         * */
        getAllItem () {
          getAllCompletionPayItem({page: 1, limit: 10000}).then((res) =>{
         ...
         */
        clickSearchFilter() {
          this.$refs.refBasicSearch.searchFilter(this.$refs.refCompletionPayTableData, this.modelSearchFilter, () => {
            this.getAllCompletionPayData();
          });
        },
        /**
