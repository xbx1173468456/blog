<template>
  <div class="completionPayDetail-container">
    <basic-handler>
    ...
    <el-collapse v-model="activeNames">
      <el-collapse-item title="基本信息" name="1">
        <el-form ref="refCompletionPayForm" class="form-container" :model="completionPayForm" label-width="120px"
          ...
          <el-row :gutter="5">
            ...
            <el-col :span="8">
              <el-form-item label="单据编码">
                <el-input size="small" clearable v-model="completionPayForm.code" :disabled="true" placeholder="系统自动生成单据编码"/>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="款项名称" prop="name" hide-required-asterisk>
                <el-input size="small" clearable v-model="completionPayForm.name"
          ...
          <el-row :gutter="5">
            <el-col :span="8">
              <el-form-item label="项目名称" prop="proName" hide-required-asterisk>
                <el-input size="small" v-model="completionPayForm.proName"
                          readOnly
            ...
            <el-col :span="8">
              <el-form-item label="分包单位" prop="enterpriseName" hide-required-asterisk>
                <el-input size="small" v-model="completionPayForm.enterpriseName"
                          readOnly
            ...
            <el-col :span="8">
              <el-form-item label="合同名称" prop="contractName" hide-required-asterisk>
                <el-input size="small"  v-model="completionPayForm.contractName"
                          readOnly
            ...
            <el-col :span="8">
              <el-form-item label="应付总额">
                <basic-input v-if="priceDisabledFlag == false" :formatValue="formatToMoney" :format="formatToThousands"
                             allowNegative
                             clearable
                             v-model="completionPayForm.shouldPayPriced"
                             placeholder="请输入应付总额"
                             :disabled="priceDisabledFlag"
                ></basic-input>
                <basic-input v-else :formatValue="formatToMoney" :format="formatToThousands"
            ...
            <el-col :span="8">
              <el-form-item label="应付日期">
                <el-date-picker size="small"
                                :editable = "false"
                                :clearable = "false"
                                v-model="completionPayForm.shouldPayDate"
                                type="date"
                                :disabled="disabledFlag"
            ...
            <el-col :span="24">
              <el-form-item label="补充说明">
                <el-input size="small" clearable
                          ...
                          :max="5000"
                          v-model="completionPayForm.remark" type="textarea" placeholder="请输入补充说明，不超过200字"/>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="5">
            <el-col :span="24">
              <el-form-item label="附件" class="add-border file">
               <basic-upload ref="refBasicUpload" :disabled="disabledFlag" :formId="completionPayFormId">
                </basic-upload>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <basic-handler class="margin-top10">
      <el-button v-if="subStagePayBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                 :disabled="disabledFlag"
                 @click="handleImportSubStagePay" >引入阶段结算单
      </el-button>
    ...
    <basic-table ref="refCompletionPayTableData"
                 :isPagination ="false"
    ...
    >
      ...
      <template slot-scope="obj" slot="unpayPrice">
        {{formatToThousands(obj.scope.row.unpayPrice)}}
      </template>
...
<script>
  ...
  export default {
    ...
    activated() {
      this.$utilsBasic.loadAddAndDetailData(this, () => {
        if(this.$route.params) {
          this.completionPayFormId = this.$route.params.id ? this.$route.params.id : undefined;
        }
        if(this.$route.query) {
          this.completionPayForm.proId = this.$route.query.proId ? this.$route.query.proId : undefined;
          this.completionPayForm.proName = this.$route.query.name ? this.$route.query.name: undefined;
          this.completionPayForm.proCode = this.$route.query.proCode ? this.$route.query.proCode: undefined;
        }
        this.handleAddBtn();
    ...
    computed: {
      ...
      priceDisabledFlag () {
        return (this.completionPayTableData.data && this.completionPayTableData.data.length > 0) || this.disabledFlag
      },
      ...
      unpayPrice:{
        get () {
          ...
          let punishSum = this.completionPayTableData.data.filter(item => {return item.billType === 6}).map( item => item.unpayPrice).reduce(reducer,0)
          let otherSum = this.completionPayTableData.data.filter(item => {return item.billType !== 6}).map( item => item.unpayPrice).reduce(reducer,0)
      ...
      shouldPayPriced:{
        get () {
          const reducer = (accumulator, currentValue) => accumulator + Number(currentValue);
          //扣罚加总 扣罚单类型billType = 6
          let punishSum = this.completionPayTableData.data.filter(item => {return item.billType === 6}).map( item => item.shouldPayPrice).reduce(reducer,0)
          let otherSum = this.completionPayTableData.data.filter(item => {return item.billType !== 6}).map( item => item.shouldPayPrice).reduce(reducer,0)
          console.log('结算金额',punishSum,otherSum)
          return otherSum - punishSum
        },
        set() {}
      ...
      qualityAssurancePrice:{
        get () {
         if(this.priceDisabledFlag){
           return this.shouldPayPriced * this.completionPayForm.qualityAssuranceScale
         }else{
           return this.completionPayForm.shouldPayPriced * this.completionPayForm.qualityAssuranceScale
         }
    ...
    methods: {
      ...
      handleDeleteBtn() {
        this.$confirm(staticDataBasic.info.delete, '提示', {
          ...
          if(this.$utils.isEmpty(this.completionPayFormId)){
            ...
            this.deleteCompletionPayData().then( () =>{
              this.$set(this.completionPayForm,'proId','')
              this.$set(this.completionPayForm,'proName','')
              this.$set(this.completionPayForm,' proCode','')
              this.addCompletionPay()
      ...
      handleAuditBtn() {
        this.$confirm(staticDataBasic.info.audit, '提示', {
          ...
          this.validateAfter(()=>{
            this.saveCompletionPayData(true).then(() => {
      ...
      handleDeleteSubListBtn() {
        this.$confirm(staticDataBasic.info.delete, '提示', {
          ...
          this.deleteByIdData(this.completionPayTableData.data, this.currentRow,(allData,data)=>{
      ...
      saveCompletionPayData (boolean) {
        ...
        return saveCompletionPay(this.completionPayForm).then(res =>{
      ...
      deleteCompletionPayData() {
        return deleteCompletionPay(this.completionPayFormId).then((res) => {
          if (res.status && res.status === 200) {
            this.$message.success(res.message)
          } else {
            this.$message({message: res.message, type: 'warning'})
      ...
      auditCompletionPayData () {
        return auditCompletionPay(this.completionPayForm).then((res) => {
      ...
      getCompletionPay () {
        if(this.$utils.isEmpty(this.completionPayFormId)){
          ...
          return getCompletionPay(this.completionPayFormId).then(res =>{
            if(res.status && res.status === 200) {
      ...
      toipClick(type) {
        ...
        if (type == 'contract' && !(this.completionPayForm.proId && this.completionPayForm.enterpriseId ))
        ...
        switch (this.type) {
          case 'project':
            if(this.completionPayForm.proId !== row.id){
              ...
              this.completionPayTableData.data = []
              this.$set(this.completionPayForm,'detailList',this.completionPayTableData.data)
          ...
          case 'counterpart':
            if(this.completionPayForm.enterpriseId !== row.id){
              ...
              this.completionPayTableData.data = []
              this.$set(this.completionPayForm,'detailList',this.completionPayTableData.data)
              console.log('enterpriseName',this.completionPayForm.enterpriseName)
            }
      ...
      getCompletionPayDetailData() {
        if(this.$utils.isEmpty(this.completionPayFormId)) {
          ...
          return getCompletionPayDetail(this.modelSearchFilter).then((res) => {
            if (res.status && res.status === 200) {
      ...
      deleteCompletionPayDetailData() {
        return deleteCompletionPayDetail(this.currentRow.id).then((res) => {
          if (res.status && res.status === 200) {
            this.$message.success(res.message)
          } else {
            this.$message({message: res.message, type: 'warning'})
      ...
      selectedDataPushTable(SelectionChangeData,type){
        if(SelectionChangeData && SelectionChangeData.length > 0){
          ...
          if(this.completionPayTableData.data && this.completionPayTableData.data.length > 0) {
            codesArr = this.completionPayTableData.data.filter(item =>item.billType == type ).map(item => item.billCode)
            for(const item of SelectionChangeData) {
              if(codesArr.indexOf(item.code) < 0){ //判断返回数据与原有数据是否有重复
                let rowObj = Object.assign({},{
                  ...
                  billCode:item.code,
                  billName:item.name,
                  unpayPrice:(item.totalPrice - item.payedPrice) || (item.payPrice - item.payedPrice) || (item.price  - item.payedPrice) , //未付：单据金额 - 结算金额
                  shouldPayPrice:(item.totalPrice - item.payedPrice) || (item.payPrice - item.payedPrice) || (item.price  - item.payedPrice) //应付 = 未付
                })
            ...
            for(const item of SelectionChangeData) {
              let rowObj = Object.assign({},{
                ...
                billCode:item.code,
                billName:item.name,
                unpayPrice:(item.totalPrice - item.payedPrice) || (item.payPrice - item.payedPrice) || (item.price  - item.payedPrice) ||(item.price - item.shouldPayPrice), //未付：单据金额 - 结算金额
                shouldPayPrice:(item.totalPrice - item.payedPrice) || (item.payPrice - item.payedPrice) || (item.price  - item.payedPrice) ||(item.price - item.shouldPayPrice) //应付 = 未付
              })
              addNewArr.push(rowObj)
            }
          }
          this.completionPayTableData.data = addNewArr.concat(this.completionPayTableData.data)
          this.$set(this.completionPayForm,'detailList',this.completionPayTableData.data)
        }
      },
      /**
      ...
      handleSubStagePayOk(){
        this.subStagePay.visibleSync = false
        //数据加入到列表中
        const subStagePaySelectionChange = this.subStagePay.subStagePaySelectionChange
