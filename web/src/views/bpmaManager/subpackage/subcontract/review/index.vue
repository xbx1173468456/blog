<template>
  <basic-split-pane class="bpmaManager-container-list review-container"
                    :default-percent='75'
                    :min-percent='25'
  >
        ...
        <div class="left-container">
          ...
          <div class="handler-container">
              <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                         @click="addClick('add')">新增评审单</el-button>
          ...
          <pagination  :total="total" :page.sync="modelSearchFilter.page" :limit.sync="modelSearchFilter.limit" @pagination="getAllreview"/>
        </div>
        </template>
        <template slot="rightContainer">
          <div class="right-container">
            <page-select-filter :selectData="selectData" :data="reviewData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
          </div>
        </template>
  ...
  export default {
    ...
    methods: {
      ...
      addClick(param) {
        ...
        if (param === 'add') {
          if (!this.$utils.isEmpty(this.proData)) {
            ...
            this.$message({message: '当前无项目，无法新增评审单!', type: 'warning'})
          }
        } else {
          this.$router.push({name: 'ReviewDetail', params: {id: this.currentRow.id}})
        }
      },
      /**
       * 获取分包合同评审列表所有数据
      ...
      getAllreview () {
        ...
        getAllSubContractReview(this.modelSearchFilter).then( (res) => {
          if(res.status && res.status === 200){

            this.tableData = res.data.rows;
            this.total = res.data.total;
            console.log('获取分包合同评审列表所有数据',res)
          }else{
       ...
       * */
      getAllItem () {
        getAllSubContractReviewItem({page: 1, limit: 10000}).then((res) =>{

          if(res.status && res.status === 200){
            console.log('获取分包合同评审项目所有数据',res)

            //this.reviewData.data = res.data.rows;
            this.$set(this.reviewData,'data',res.data.rows)
            console.log('reviewData',this.reviewData.data)
          }else{
            this.$message({message: res.message,type: 'warning'})
          }
        }).catch(error => {
