<template>
  <div class="personDialog">
    <basic-dialog v-el-drag-dialog :title="title" :visible.sync="dialogTableVisible">
      ...
      <div v-if="this.type ==3||this.type ==4">
        <tableItem2
              :columns="columns"
              :tableData="tableData"
              :page="page"
...
export default {
  ...
  methods: {
    ...
    save() {
      if (this.type == 3) {
        for(var i=0;i<this.$store.getters.getselectP.length;i++){
          this.$store.getters.getselectP.mid=this.$store.getters.getselectP.masterId
        }
