    ...
    <template slot="leftContainer">
        <div class="left-container">
            ...
            <basic-table ref="refCheckTable"
              ...
              <template slot-scope="obj" slot="stockPrice">
                {{formatToThousandsFour(obj.scope.row.stockPrice)}}
              </template>
              <template slot-scope="obj" slot="checkPrice">
                {{formatToThousandsFour(obj.scope.row.checkPrice)}}
              </template>
              <template slot-scope="obj" slot="profitLossPrice">
                {{formatToThousandsFour(obj.scope.row.profitLossPrice)}}
              </template>
            </basic-table>
        </div>
    </template>
...
export default {
  ...
  created() {
    ...
    methods: {
            ...
            getList(this.modelSearchFilter).then(res => {
                if (res.status == 200) {
                  ...
                  this.$nextTick(function () {
                    if (this.$refs.refCheckTable) {
                      this.$refs.refCheckTable.setCurrentRow(this.tableData.data[0]);
                    }
                  });
                }
            }).catch(err => {
         ...
         */
        clickSearchFilter() {
        this.$refs.refBasicSearch.searchFilter(this.$refs.refCheckTable, this.modelSearchFilter, () => {
            this.getSelectedProjectData();
        });
        },
        //添加
        ...
        showDetail() {
            ...
            this.$router.push({
                path: '/bpmaManager/goods/inventory/check/components/checkDetail',
                name: 'checkDetail',
                params: {
                    id: this.rowId
                }
