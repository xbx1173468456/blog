<template>
  <div>
    <el-table ref="singleTable" :data="tableData" tooltip-effect="dark" border :height="heightUserManager"
      ...
      <el-table-column show-overflow-tooltip :align="col.align" header-align="center" v-for="(col,i) in columns" :label="col.text"
    ...
    methods: {
      ...
      formatter(scope, col) {
        if (col.value == 'validateStatus') {
          ...
          let v = '' + scope.row[col.value]
          if (!scope.row[col.value]) return scope.row[col.value]
          if (v.indexOf('.') < 0) {
            return scope.row[col.value] + '.00'
          } else {
            return scope.row[col.value]
          ...
          return scope.row.offerExpiryDate ? scope.row.offerExpiryDate.split(' ')[0] : ''
        } else {
          return scope.row[col.value]
        }
      }
