    ...
    <div>
        <el-table
            ...
            <template slot-scope="scope" >
                        <div v-if="!col.showTable && !col.select">
                            {{formatter(scope,col)}}
                        </div>
                        <div v-if="col.showTable" class="tableClass">
                            <el-form :model="tableData[scope.$index]" ref="'formModel' + scope.$index" label-width="120px" class="demo-ruleForm">
                                <el-form-item :prop="col.value">
                                    <el-input clearable size="small" v-model="scope.row[col.value]" maxlength="100" :ref="col.value" :disabled="col.disable" :type="col.types?'number':'text'" @blur="calculate(scope.row,col,$event)" :max="col.max" :min="col.min"></el-input>
                                </el-form-item>
                            </el-form>
                        </div>
                        <div v-if="col.select" class="tableClass">
                            <el-select v-model="scope.row[col.value]" placeholder="请选择">
                                <el-option
    ...
    methods:{
        ...
        calculate(scope,col,$event){
            ...
            if(col.types){
                ...
                if(scope.unitPrice<0){
                    scope.unitPrice=0
                    Math.Number()
                }
                if(scope.invoiceTaxRate<0){
                    scope.invoiceTaxRate=0
                }
                if(scope.invoiceTaxRate>1){
                    scope.invoiceTaxRate=1
                }
