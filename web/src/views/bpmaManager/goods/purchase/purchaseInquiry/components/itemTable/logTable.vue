                        ...
                        <div v-if="col.showTable" class="tableClass">
                            <el-form :model="tableData[scope.$index]" ref="'formModel' + scope.$index" label-width="120px" class="demo-ruleForm">
                                <el-form-item :prop="col.value">
                                    <el-input clearable size="small" v-model="scope.row[col.value]" maxlength="100" :ref="col.value" :disabled="col.disable" :type="col.types?'number':'text'" @blur="calculate(scope.row,col,$event)" :min="col.min"></el-input>
                                </el-form-item>
                            </el-form>
                        </div>
    ...
    methods:{
        calculate(scope,col,$event){
            ...
            if(col.types){
                ...
                scope.applySum=Number(scope.applySum).toFixed(4)
                scope.maxPrice=Number(scope.maxPrice).toFixed(4)
            }
        },
        currentPage(pages){
