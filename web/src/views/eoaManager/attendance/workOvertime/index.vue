<template>
  <div class="listManager-container">
    <div class="handler-container">
      ...
      <el-button type="primary" plain @click="lookTetails" :disabled="status" size="small" icon="el-icon-if-see">
        查看
      </el-button>
      <el-button type="primary" icon="el-icon-refresh" plain @click="getDataList" size="small">
        刷新
      </el-button>
      <el-form :inline="true" class="right">
        <el-form-item>
          <el-input placeholder="请输入关键字" v-model="search.fuzzy"
                    @keyup.enter.native.prevent="searchDatalist(search.fuzzy)" size="small">
    ...
    <el-table
      ...
      <el-table-column
        ...
        align="center"
        width="140"
        label="审核状态" :formatter="formatAuditStatusEnum">
      </el-table-column>
      <el-table-column
        property="code"
        align="center"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*获取请假列表*/
      getDataList() {
        this.$store.dispatch('GetOvertimeList', this.search).then((response) => {
      ...
      addTetails() {
        ...
        this.$router.push({name: 'workOvertimeDetails', params: {id: 0}})
      },
      lookTetails() {
        this.$store.dispatch("cacheId", "0");
        this.$router.push({name: 'workOvertimeDetails', params: {id: this.row.id}, query: {id: this.row.id}})
      },
      /*分页查询*/
