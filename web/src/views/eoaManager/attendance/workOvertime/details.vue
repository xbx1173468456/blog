<template>
  <div class="app-container detailsManager-container">
    <div class="handler-container">
      <el-button v-if="isUpdate" type="primary" plain @click="resetData" icon="el-icon-plus" size="small">新增</el-button>
      <el-button type="primary" :disabled="auditStatus" plain @click="changeDatalist" icon="el-icon-if-save"
        size="small">保存</el-button>
      <el-button v-if="isUpdate" type="primary" :disabled="auditStatus || !formData.id" plain @click="deleteHint" icon="el-icon-delete" size="small">删除
      </el-button>
      <el-button v-if="isUpdate" type="primary" :disabled="auditStatus" plain @click="auditHint" icon="el-icon-if-auth" size="small">审核
      </el-button>
      <el-button type="primary" v-if="formData.processInstanceId && isUpdate" plain @click="lookData" icon="el-icon-if-auth"
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <basic-form-item label="审核状态">
            <basic-input v-model="formData.validateStatus" :format="formatAuditStatusEnum" disabled></basic-input>
          </basic-form-item>
        </el-col>
        <el-col :span="8">
        ...
        <!-- <el-col :span="8">
          <basic-form-item label="加班开始日期">
            <el-date-picker :disabled="auditStatus" placeholder="请选择日期" size="small" type="datetime"
              v-model="formData.beginTime" value-format="timestamp" @change="$set(formData, 'endTime', undefined)">
            </el-date-picker>
          </basic-form-item>
        </el-col>
        <el-col :span="8">
          <basic-form-item label="加班结束日期">
            <el-date-picker :disabled="auditStatus || !formData.beginTime" :picker-options="pickerOptions"
              placeholder="请选择日期" size="small" type="datetime" v-model="formData.endTime"
              value-format="timestamp">
            </el-date-picker>
          </basic-form-item>
        </el-col> -->
        <el-col :span="16">
          <el-form-item label="加班时间" prop="timeData">
            <el-date-picker size="small" v-model="formData.timeData" :editable="false" type="datetimerange"
              range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" format="yyyy-MM-dd HH:mm"
              @change="formatTime" :disabled="auditStatus" :picker-options="pickerOptions">
            </el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :span="8">
        ...
        <el-col :span="24">
          <basic-form-item label="文档附件" class="add-border file">
            <basic-upload ref="refBasicUpload" :formId="formData.id" :disabled="auditStatus">
            </basic-upload>
          </basic-form-item>
        </el-col>
        <el-col :span="16">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        pickerOptions: {
          onPick({ maxDate, minDate }) {
            // return time.getTime() < that.formData.beginTime
    ...
    methods: {
      /*获取详情*/
      getDatalist(id) {
        this.$store.dispatch('GetOvertimeDetails', id).then((response) => {
          response.data.beginTime = response.data.beginTime ? new Date(response.data.beginTime).getTime() : ''
          response.data.endTime = response.data.endTime ? new Date(response.data.endTime).getTime() : ''
      ...
      changeDatalist(lock) {
        return new Promise((reslove, reject) => {
          this.$refs.formData.validate((valid) => {
            if (valid) {
              if (this.formData.id) {
                this.formatTime(this.formData.timeData)
                this.$store.dispatch('AmendOvertime', this.formData).then((response) => {
                  if (lock != true) {
                ...
                this.formatTime(this.formData.timeData)
                this.$store.dispatch('AddOvertimeList', this.formData).then((response) => {
                  if (lock != true) {
                    this.$message.success(response.message);
                  }
                  if(!this.$refs.refBasicUpload.submitUpload(response.data.id)) return false;
      ...
      /*删除方法*/
      deleteDatalist() {
        this.$store.dispatch('DeleteOvertime', this.formData.id).then((response) => {
          this.$message.success(response.message);
          this.resetData();
        }).catch(() => {
        })
        ...
        this.$refs.formData.validate((valid) => {
          if (valid) {
            this.$confirm(' 亲，您是否确定审核当前数据，审核后数据不能修改！', '提示', {
              ...
              res.data.beginTime = res.data.beginTime ? new Date(res.data.beginTime).getTime() : ''
              res.data.endTime = res.data.endTime ? new Date(res.data.endTime).getTime() : ''
      ...
      /*提交审核*/
      auditDatalist() {
        this.$store.dispatch('AuditOvertime', this.formData).then((response) => {
          response.data.beginTime = response.data.beginTime ? new Date(response.data.beginTime).getTime() : ''
          response.data.endTime = response.data.endTime ? new Date(response.data.endTime).getTime() : ''
          this.formData = response.data
          this.copyValue()
      ...
      /*处理赋值的方法*/
      copyValue() {
        this.$set(this.formData, 'timeData', [new Date(this.formData.beginTime), new Date(this.formData.endTime)])
        if (this.formData.validateStatus == 1 || this.formData.validateStatus == 3 || this.formData.validateStatus == 4) {
      ...
      formatTime(time) {
        if(time) {
          this.$set(this.formData, 'beginTime', new Date(time[0]).getTime())
          this.$set(this.formData, 'endTime', new Date(time[1]).getTime())
          this.$set(this.formData, 'timeData', time)
        }
      }
    }
  }
