<template>
  <div class="workDetail-container-list">
    <div class="grid-content bg-purple handler-container">
      <el-button type="primary" size="small" icon="el-icon-plus" @click="add()" plain>新增新闻</el-button>
    ...
    <div class="logForm">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm form-container">
        <el-row :gutter="10">
          <el-col :span="8">
            <basic-form-item label="审核状态" prop="validateStatus">
              <el-select v-model="ruleForm.validateStatus" placeholder="" disabled size="small">
                <el-option v-for="item in this.dictionary.validateStatus" :key="parseInt(item.code)" :label="item.name"
          ...
          <el-col :span="8">
            <basic-form-item label="发布机构" prop="sendOrg ">
              <el-input clearable size="small" v-model="ruleForm.sendOrg" :disabled="isAudit" maxlength="100">
              </el-input>
            </basic-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="16">
            <basic-form-item label="新闻标题" prop="title">
              <el-input clearable size="small" v-model="ruleForm.title" :disabled="isAudit" maxlength="100"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="8">
            <basic-form-item label="新闻文号" prop="number">
              <el-input clearable size="small" v-model="ruleForm.number" :disabled="isAudit" maxlength="100"></el-input>
            </basic-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <basic-form-item label="正文内容" prop="content">
              <el-input type="textarea" :disabled="isAudit" maxlength="200" :autosize="{ minRows: 3, maxRows: 5}"
                placeholder="请输入内容" v-model="ruleForm.content">
              </el-input>
            </basic-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="24">
            <basic-form-item label="补充说明" prop="remark ">
              <el-input type="textarea" :disabled="isAudit" maxlength="100" :autosize="{ minRows: 3, maxRows: 5}"
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="8">
            <basic-form-item label="登记时间" prop="inputTime">
              <el-date-picker size="small" :editable="false" v-model="ruleForm.inputTime" type="date" format="yyyy-MM-dd" disabled
                placeholder="选择日期">
              </el-date-picker>
            </basic-form-item>
          </el-col>
...
<script>
  ...
  import { addNews, showDetail, auditNews, deleteNews, modifyNews } from '@/api/eoaManager/news/index.js'
    ...
    methods: {
      ...
      save(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            // return
            if (this.logId) {
              modifyNews(this.ruleForm).then(res => {
                if (res.status == 200) {
      ...
      audit() {
        this.$confirm('亲，您是否确定审核当前数据，审核后数据不能修改！', {
          ...
          auditNews(this.ruleForm).then(res => {
            if (res.status == 200) {
              this.$message.success(res.message)
              this.getDetail()
            }
