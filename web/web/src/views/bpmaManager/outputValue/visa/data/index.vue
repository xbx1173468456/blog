<template>
  <basic-container class="bpmaManagerOutputValueVisaData-container-list ">
      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增资料</el-button>
...
<script>
  ...
  import { getProjectList, getVisafileMaster } from '@/api/bpmaManager/outputValue/visa/data'
  ...
  export default {
    ...
    methods: {
      ...
      getVisafileMaster() {
        ...
        getVisafileMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getVisafileMaster()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增资料！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/outputValue/visa/data/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'ifFinish':
            return this.dataEnum.isOrNot.filter(item => item.code == row.ifFinish)[0].name
          case 'finishDate':
            if (!row.finishDate) return
            return row.finishDate ? row.finishDate.split(' ')[0] : ''
          case 'handleDate':
            if (!row.handleDate) return
