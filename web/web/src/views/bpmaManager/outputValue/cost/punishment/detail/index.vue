<template>
  <div :style="windowHeight" class="addFrom">
    <basic-handler>
      <el-button @click="initProjectFormData" icon="el-icon-plus" plain size="small" type="primary">新增扣罚</el-button>
    ...
    <el-form :model="formData" :rules="rules" class="form-container" ref="form">
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <basic-form-item label="扣罚名称" prop="name">
            <basic-input :disabled="disabled" placeholder="请输入扣罚名称" v-model="formData.name"></basic-input>
          </basic-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <basic-form-item label="扣罚金额" prop="price">
            <basic-input :disabled="disabled" :format="formatToThousands" :formatValue="formatToMoney" placeholder="请输入扣罚金额"
      ...
      <basic-form-item label="扣罚事由">
        <el-input :disabled="disabled" :maxlength="5000" :rows="textareaHeight" placeholder="请输入扣罚事由" type="textarea"
...
<script>
  ...
  import {
    deleteValuePunishMaster,
    reviewValuePunishMaster,
    saveValuePunishMaster,
    poatValuePunishMaster,
    getValuePunishMasterDetail
  } from '@/api/bpmaManager/outputValue/cost/punishment.js'
  ...
  export default {
    ...
    activated() {
      this.$utilsBasic.loadAddAndDetailData(this, () => {
        ...
        if (this.$route.params.detail != 'detail') {
          await this.getValuePunishMasterDetail(this.$route.params.detail)
        } else {
          this.initProjectFormData()
        }
    ...
    methods: {
      ...
      getValuePunishMasterDetail(id) {
        return getValuePunishMasterDetail(id).then(res => {
          if (res.data) {
      ...
      initFormData(lock) {
        ...
        if (lock) {
          this.initProjectFormData()
          this.$router.replace({
            path: `/bpmaManager/outputValue/cost/punishment/id/0/detail`,
            query: proData
          })
        } else {
          this.$router.replace({
            path: `/bpmaManager/outputValue/cost/punishment/id/0/${this.formData.id}`,
            query: proData
          })
        }
      },
      deleteData() {
        if (this.formData.id) {
          this.$confirm('亲，您是否确定删除当前数据，删除后数据不能恢复！', '提示', {
            ...
            deleteValuePunishMaster(this.formData.id).then(res => {
              if (res.rel) {
      ...
      review() {
        this.$confirm(' 亲，您是否确定审核当前数据，审核后数据不能修改！', '提示', {
          ...
          reviewValuePunishMaster(this.formData).then(res => {
            if (res.rel) {
              this.isUpdate = true
              this.formData = res.data
              this.formData.handleDate = new Date(this.formData.handleDate).getTime()
      ...
      save(lock) {
        return new Promise((reslove, reject) => {
          ...
          this.$refs.form.validate(async valid => {
            ...
            if (this.formData.id) {
              res = await poatValuePunishMaster(this.formData)
            } else {
              res = await saveValuePunishMaster(this.formData)
            }
            if (!res.rel) return
            this.formData = res.data
