      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增奖励</el-button>
...
<script>
  ...
  import { getProjectList, getValueAwardMasters } from '@/api/bpmaManager/outputValue/cost/reward.js'
  ...
  export default {
    ...
    methods: {
      ...
      getValueAwardMasters() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getValueAwardMasters(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getValueAwardMasters()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增奖励！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/outputValue/cost/reward/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'price':
            return this.$utilsBasic.toMoney(row.price)
          case 'handleDate':
