<template>
  <basic-split-pane class="biddingAnnouncement-container "
                    :default-percent='75'
                    :min-percent='25'
  >
        ...
        <div class="left-container">
          ...
          <div class="handler-container">
              <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                         @click="addClick('add')">新增公告</el-button>
          ...
          <pagination :total="total" :page.sync="modelSearchFilter.page" :limit.sync="modelSearchFilter.limit" @pagination="getAllbiddingAnnouncement"/>
        </div>
        </template>
        <template slot="rightContainer">
          <div class="right-container">
            <page-select-filter :selectData="selectData" :data="announcementData"  v-model="selectValue" @change="announcementChange" @refreshData="getAllItem"></page-select-filter>
          </div>
        </template>
...
<script>
  ...
  export default {
    ...
    activated(){
      this.getAllbiddingAnnouncement()
    },
    created() {
    ...
    methods: {
      ...
      addClick(param) {
        ...
        if (param === 'add') {
          ...
          this.$router.push({
            ...
            this.$message({message:'当前无项目，无法新增公告!',type:'warning'})
          }
        } else {
          this.$router.push({name: 'BiddingAnnouncementDetail', params: {id: this.currentRow.id}})
        }
      },
      /**
       * 获取招标公告列表所有数据
      ...
      getAllbiddingAnnouncement () {
       // console.log('this.modelSearchFilter',this.modelSearchFilter)
          this.$store.dispatch('GetAllbiddingAnnouncement',this.modelSearchFilter).then( (res) => {
            if(res.status && res.status === 200){

              this.tableData = res.data.rows;
              this.total = res.data.total;
              console.log('获取招标公告列表所有数据',res)
            }else{
      ...
      * */
      getAllItem () {
      this.$store.dispatch('GetAllItem',{page: 1, limit: 10000}).then((res) =>{

          if(res.status && res.status === 200){
            console.log('获取招标项目所有数据',res)

            this.$set(this.announcementData,'data',res.data.rows)
           // this.announcementData.data = res.data.rows;
            console.log('announcementData',this.announcementData.data)
          }else{
            this.$message({message: res.message,type: 'warning'})
          }
        }).catch(error => {
      ...
      * */
      getStage () {
        this.$store.dispatch('GetDictionaryDataByTypeFilter','SYS_PRO_STAGE')
          .then( (res) => {
            if(res.status && res.status === 200){
