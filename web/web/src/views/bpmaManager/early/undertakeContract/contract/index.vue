...
<script>
  ...
  import { getProjectList, getRegistrationList } from '@/api/bpmaManager/early/undertakeContract/registration.js'
  ...
  export default {
    ...
    methods: {
      ...
      getRegistrationList() {
        ...
        getRegistrationList(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/early/undertakeContract/contract/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
