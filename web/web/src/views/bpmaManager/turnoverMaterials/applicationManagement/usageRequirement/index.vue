<template>
  <basic-split-pane
    class="usageRequirementIndex-container-index"
    :default-percent="75"
    :min-percent="25"
  >
    <template slot="leftContainer">
      <div class="left-container">
        ...
        <basic-table
          ...
          :dataTable="tableData"
          :columns="columnsData.indexTitle"
          @current-change="currentChangeRow"
          :pagination="modelSearchFilter"
          @pagination="getUsageRequirement"
        >
          <template slot-scope="obj" slot="validateStatus">{{formatValidateStatus(obj.scope.row)}}</template>
          <template
            slot-scope="obj"
            slot="useEnterpriseType"
          >{{formatUseEnterpriseType(obj.scope.row)}}</template>
          <template slot-scope="obj" slot="handleDate">{{formatDate(obj.scope.row.handleDate)}}</template>
          <template
            slot-scope="obj"
            slot="useTotalPrice"
          >{{formatToThousands(obj.scope.row.useTotalPrice)}}</template>
        </basic-table>
      </div>
    </template>
...
// import { getList } from "@/api/bpmaManager/goods/putStorage/outTheFront/index.js";
import { getList } from "@/api/bpmaManager/turnoverMaterials/applicationManagement/usageRequirement/index.js";
export default {
  name: "usageRequirementIndex",
  mixins: [computed, methods, formatBasic, formatEnum],
  activated() {
    this.getUsageRequirement();
  },
  created() {
    //初始化数据字典
    this.getEnumData(["GetValidateStatusData", "GetOutTypeData"]);
    this.selectItems();
    this.getDic();
  },
  data() {
