    ...
    <template slot="leftContainer">
        <div class="left-container">
            ...
            <basic-table ref="refOtherTable"
                ...
                <template slot-scope="obj" slot="outType">
                    {{formatOutType(obj.scope.row)}}
                </template>
            </basic-table>
        </div>
    </template>
...
export default {
  ...
  created() {
    ...
    methods: {
            ...
            getList(this.modelSearchFilter).then(res => {
                if (res.status == 200) {
                  ...
                  this.$nextTick(function () {
                    if (this.$refs.refOtherTable) {
                      this.$refs.refOtherTable.setCurrentRow(this.tableData.data[0]);
                    }
                  });
                }
            }).catch(err => {
         ...
         */
        clickSearchFilter() {
        this.$refs.refBasicSearch.searchFilter(this.$refs.refOtherTable, this.modelSearchFilter, () => {
            this.getSelectedProjectData();
        });
        },
        //添加
        ...
        showDetail() {
            ...
            this.$router.push({
                path: '/bpmaManager/goods/outbound/other/components/otherDetail',
                name: 'otherDetails',
                params: {
                    id: this.rowId
                }
