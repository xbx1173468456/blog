...
<basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
    <template slot="leftContainer">
        <div class="left-container">
            ...
            <basic-table ref="refTransfersTable"
...
export default {
    ...
    methods: {
        ...
        changeProject(obj) {
            if(!obj) return
            this.proObj = obj
            this.modelSearchFilter.outProId=this.proObj ? this.proObj.id : ''
            ...
            getList(this.modelSearchFilter).then(res => {
                if (res.status == 200) {
                  ...
                  this.$nextTick(function () {
                    if (this.$refs.refTransfersTable) {
                      this.$refs.refTransfersTable.setCurrentRow(this.tableData.data[0]);
                    }
                  });
                }
            }).catch(err => {
         ...
         */
        clickSearchFilter() {
        this.$refs.refBasicSearch.searchFilter(this.$refs.refTransfersTable, this.modelSearchFilter, () => {
            this.getSelectedProjectData();
        });
        },
        //添加
      addClick() {
        if(!this.modelSearchFilter.outProId){
        ...
        showDetail() {
            ...
            this.$router.push({
                path: '/bpmaManager/goods/transfers/outbound/components/outboundDetail',
                name: 'outboundDetail',
                params: {
                    id: this.rowId
                }
