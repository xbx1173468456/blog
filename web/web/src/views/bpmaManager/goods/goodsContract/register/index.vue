      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增合同</el-button>
...
<script>
  ...
  import { getProjectList, getPayContractInfoMaster } from '@/api/bpmaManager/goods/goodsContract/register'
  ...
  export default {
    ...
    methods: {
      ...
      contractChange(data) {
        ...
        this.getPayContractInfoMaster()
      },
      getPayContractInfoMaster() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getPayContractInfoMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增合同登记！')
        ...
        this.$router.push({
          path: `/bpmaManager/goods/goodsContract/register/id/${this.formData.id || 0}`,
          query: {
            formData
          }
        })
