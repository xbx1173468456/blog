                        ...
                        <div v-if="col.showTable" class="tableClass">
                            <el-form :model="tableData[scope.$index]" ref="'formModel' + scope.$index" label-width="120px" class="demo-ruleForm">
                                <el-form-item :prop="col.value">
                                    <el-input clearable size="small" v-model="scope.row[col.value]" maxlength="100" :ref="col.value" :disabled="col.disable" :type="col.types?'number':'text'" @blur="calculate(scope.row,col,$event)" @keyup.native="proving1(scope.row,col,$event)"></el-input>
                                </el-form-item>
                            </el-form>
                        </div>
    ...
    methods:{
        ...
        getTransportMode() {
          getDictionaryDataByTypeFilter('SYS_SEND_GOODS')
            .then((res) => {
              console.log('SYS_SEND_GOODS',res)
              if (res.rel) {
                this.transportMode = res.data;
              } else {
