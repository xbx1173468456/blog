    ...
    <template slot="leftContainer">
      <div class="left-container">
        <div class="handler-container">
          <el-button type="primary" size="small" icon="el-icon-plus" @click="addLog()" plain>新增采购申请</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //查询左边
      select(currentPage, limit, fuzzy) {
        if (!this.proObj) {
          return
        }
        let data = {
      ...
      //添加日志
      addLog() {
        if (!this.proObj || !this.proObj.id) return this.$message.warning('当前无项目，无法新增采购申请！')
        this.$store.dispatch('setTrackId', '')
        this.$store.dispatch("setSelectP", []);
        this.$store.dispatch("cacheId", '0')
        this.$router.push({
          path: '/bpmaManager/goods/purchase/purchaseApplication/components/materialApplyDetail/id',
