      ...
      <template slot="leftContainer">
        <div class="left-container">
          <div class="handler-container">
            <el-button type="primary" size="small" icon="el-icon-plus" @click="addLog()" plain>新增供方比选</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //添加日志
      addLog() {
        if (!this.proObj || !this.proObj.id) return this.$message.warning('当前无项目，无法新增供方比选！')
        this.$store.dispatch('setTrackId', '')
        this.$store.dispatch("cacheId", '0')
        this.$router.push({
          path: '/bpmaManager/goods/purchase/chooseSupplier/components/chooseSupplierDetail/id',
      ...
      //搜索
      query() {
        this.select(this.page.currentPage, this.page.limit, this.fuzzy)
      },
      //查看详情
      showDetail() {
        if (!this.rowId) {
