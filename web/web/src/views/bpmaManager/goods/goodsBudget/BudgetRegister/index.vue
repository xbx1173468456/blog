      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增预算</el-button>
...
<script>
  ...
  import { getProjectList, getMaterialBudgetMaster } from '@/api/bpmaManager/goods/goodsBudget/budgetRegister'
  ...
  export default {
    ...
    methods: {
      ...
      getMaterialBudgetMaster() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getMaterialBudgetMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getMaterialBudgetMaster()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增预算！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/goods/goodsBudget/BudgetRegister/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'budgetPrice':
            return this.$utilsBasic.toThousands(row.budgetPrice)
          case 'borrowPrice':
