<template>
  <div :style="windowHeight" class="detail-container-list">
    <basic-handler>
      <el-button @click="initFormData" icon="el-icon-plus" plain size="small" type="primary">新增还款</el-button>
    ...
    <el-form :model="formData" :rules="rules" class="form-container" ref="form">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <basic-form-item  label="还款人员" prop="handlePerson">
            <el-input :disabled="disabled" readOnly placeholder="请选择还款人员" v-model="formData.handlePerson" size="small">
      ...
      <el-row :gutter="10">
        <el-col :span="8">
          <basic-form-item  label="支付方式" prop="payMode">
            <el-select :disabled="disabled"
              @change="val => formData.payMode = payModeSelectData.filter(res => res.id == val)[0].name" placeholder="无"
        ...
        <el-col :span="8">
          <basic-form-item  label="还款金额">
            <basic-input :disabled="disabled" :format="formatToThousands" :formatValue="formatToMoney"
              placeholder="请输入还款金额" v-model="formData.payPrice" price></basic-input>
          </basic-form-item>
        </el-col>
      </el-row>
    ...
    <basic-dialog v-el-drag-dialog :visible.sync="visibleSyncUserManager" title="选择还款人员">
      <dialog-user-manager @selectionChange="selectionChange" filterValid v-if="visibleSyncUserManager">
      </dialog-user-manager>
      <div class="dialog-footer" slot="footer">
...
<script>
  ...
  import {
    deleteFinanceReturns,
    reviewFinanceReturns,
    saveFinanceReturns,
    postFinanceReturns,
    getFinanceReturnsDetail
  } from '@/api/bpmaManager/finance/borrow/repayBill.js'
  ...
  export default {
    ...
    activated() {
      this.$utilsBasic.loadAddAndDetailData(this, () => {
        ...
        if (this.$route.params.detail != 'detail') {
          await this.getFinanceReturnsDetail(this.$route.params.detail)
        } else {
          this.initProjectFormData()
        }
    ...
    methods: {
      ...
      getFinanceReturnsDetail(id) {
        return getFinanceReturnsDetail(id).then(res => {
          if (res.data) {
      ...
      initFormData(lock) {
        ...
        if (lock) {
          this.initProjectFormData()
          this.$router.replace({
            path: `/bpmaManager/finance/borrow/repayBill/id/0/detail`,
            query: proData
          })
        } else {
          this.$router.replace({
            path: `/bpmaManager/finance/borrow/repayBill/id/0/${this.formData.id}`,
            query: proData
          })
        }
      },
      deleteData() {
        if (this.formData.id) {
          this.$confirm('亲，您是否确定删除当前数据，删除后数据不能恢复！', '提示', {
            ...
            deleteFinanceReturns(this.formData.id).then(res => {
              if (res.rel) {
      ...
      review() {
        this.$confirm(' 亲，您是否确定审核当前数据，审核后数据不能修改！', '提示', {
          ...
          reviewFinanceReturns(this.formData).then(res => {
            if (res.rel) {
              this.isUpdate = true
              this.formData = res.data
              this.$message.success(res.message)
      ...
      save(lock) {
        return new Promise((reslove, reject) => {
          ...
          this.$refs.form.validate(async valid => {
            ...
            if (this.formData.id) {
              res = await postFinanceReturns(this.formData)
            } else {
              res = await saveFinanceReturns(this.formData)
            }
            if (!res.rel) return
            this.formData = res.data
