        ...
        <template slot="leftContainer">
          <div class="left-container">
            ...
            <div class="table-row table-container">
              <el-table ref="refUserManager" :height="heightUserManager" :data="data" border highlight-current-row stripe @cell-click="cellClick">
...
<script>
  ...
  import { getProjectList, getFinanceShouldIncomeQuery } from '@/api/bpmaManager/finance/collectMoney/receivable.js'
  ...
  export default {
      ...
      return {
        ...
        pickerOptions: {
          disabledDate(time) {
            ...
            return time.getTime() < new Date(that.listQuery.startTime).getTime()
          }
        },
        pickerOptions1: {
          disabledDate(time) {
            if (!that.listQuery.endTime) return
            return time.getTime() > new Date(that.listQuery.endTime).getTime()
          }
        }
      }
    },
    ...
    async created() {
      this.getEnumData(['GetIncomeTypeEnum'])
    ...
    methods: {
      ...
      getData() {
        ...
        getFinanceShouldIncomeQuery(this.modelSearchFilter).then(res => {
      ...
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'billType':
            let data = this.dataEnum.incomeType.filter(result => result.code == row.billType)[0]
      ...
      cellClick(row, column, cell, event) {
        ...
        switch(row.billType) {
          case 1:
            this.$router.push({
              path: `/bpmaManager/finance/collectMoney/registration/id/0/${row.billId}`,
              query
            })
            break
          case 2:
            this.$router.push({
              path: `/bpmaManager/outputValue/settlement/stage/id/0/${row.billId}`,
              query
            })
            break
          case 3:
            this.$router.push({
              path: `/bpmaManager/outputValue/settlement/completion/id/0/${row.billId}`,
              query
            })
            break
        }
