<template>
  <div :style="windowHeight" class="contractId-container-list">
    <basic-handler>
      <el-button @click="initFormData" icon="el-icon-plus" plain size="small" type="primary">新增发票</el-button>
    ...
    <el-form :model="formData" :rules="rules" class="form-container" ref="form">
      ...
      <el-row :gutter="10">
        <el-col :span="8">
          <basic-form-item label="发票类型" prop="invoiceTypeId">
            <el-select :disabled="disabled"
                       @change="val => formData.invoiceType = invoiceTypeSelectData.filter(res => res.id == val)[0].name"
                       placeholder="无"
        ...
        <el-col :span="8">
          <basic-form-item label="发票税率" prop="invoiceTaxRate">
            <basic-input :disabled="disabled" :formatValue="formatToMoney" :max="1" :min="0" placeholder="请输入大于0小于1的发票税率"
      ...
      <el-row :gutter="10">
        <el-col :span="8">
          <basic-form-item label="发票代码">
            <basic-input :disabled="disabled" v-model="formData.invoiceCode"></basic-input>
          </basic-form-item>
        </el-col>
        <el-col :span="8">
          <basic-form-item label="发票号码">
            <basic-input :disabled="disabled" v-model="formData.incoiceNumber"></basic-input>
          </basic-form-item>
        </el-col>
        <el-col :span="8">
          <basic-form-item label="税后金额">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <basic-form-item label="开票人员">
            <basic-input :disabled="disabled" placeholder="请输入开票人员" v-model="formData.handlePerson"></basic-input>
          </basic-form-item>
        </el-col>
      </el-row>
    ...
    <basic-dialog v-el-drag-dialog :visible.sync="visibleSyncContract" title="合同-选择" >
      <dialog-my-contract :oppositeEnterpriseId="formData.enterpriseId" :proId="formData.proId"
                           @selectionChange="selectionChange" filterValid
...
<script>
  ...
  import {
    deleteFinanceIncomeBills,
    reviewFinanceIncomeBills,
    saveFinanceIncomeBills,
    postFinanceIncomeBills,
    getFinanceIncomeBillDetail
  } from '@/api/bpmaManager/finance/collectMoney/invoiceBill.js'
  ...
  export default {
    ...
    activated() {
      this.$utilsBasic.loadAddAndDetailData(this, () => {
        ...
        if (this.$route.params.detail != 'detail') {
          await this.getFinanceIncomeBillDetail(this.$route.params.detail)
        } else {
          this.initProjectFormData()
        }
      })
    ...
    methods: {
      ...
      getFinanceIncomeBillDetail(id) {
        return getFinanceIncomeBillDetail(id).then(res => {
          if (res.data) {
      ...
      initFormData(lock) {
        ...
        if (lock) {
          this.initProjectFormData()
          this.$router.replace({
            path: `/bpmaManager/finance/collectMoney/invoiceBill/id/0/detail`,
            query: proData
          })
        } else {
          this.$router.replace({
            path: `/bpmaManager/finance/collectMoney/invoiceBill/id/0/${this.formData.id}`,
            query: proData
          })
        }
      },
      deleteData() {
        if (this.formData.id) {
          this.$confirm('亲，您是否确定删除当前数据，删除后数据不能恢复！', '提示', {
            ...
            deleteFinanceIncomeBills(this.formData.id).then(res => {
              if (res.rel) {
      ...
      review() {
        this.$confirm(' 亲，您是否确定审核当前数据，审核后数据不能修改！', '提示', {
          ...
          reviewFinanceIncomeBills(this.formData).then(res => {
            if (res.rel) {
              this.isUpdate = true
              this.formData = res.data
              this.$message.success(res.message)
      ...
      save(lock) {
        return new Promise((reslove, reject) => {
          ...
          this.$refs.form.validate(async valid => {
            ...
            if (this.formData.id) {
              res = await postFinanceIncomeBills(this.formData)
            } else {
              res = await saveFinanceIncomeBills(this.formData)
            }
            if (!res.rel) return
            this.formData = res.data
