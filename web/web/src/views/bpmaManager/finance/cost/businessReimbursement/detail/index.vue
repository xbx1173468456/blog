      ...
      <el-collapse-item name="1" title="基本信息">
        <el-form :model="formData" :rules="rules" class="form-container" ref="form" style="margin-bottom: 0;">
          ...
          <el-row :gutter="10">
            ...
            <el-col :span="8">
              <basic-form-item label="报销金额">
                <basic-input :format="formatToThousands" disabled :formatValue="formatToMoney" placeholder="请输入报销金额" v-model="applyPrice" price>
                </basic-input>
              </basic-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="10">
            ...
            <el-col :span="8">
              <basic-form-item label="退还金额">
                <basic-input :format="formatToThousands" disabled :formatValue="formatToMoney" placeholder="请输入报销金额" v-model="returnPrice" price>
                </basic-input>
              </basic-form-item>
            </el-col>
          </el-row>
          <basic-form-item label="补充说明">
            <el-input :disabled="disabled" :maxlength="5000" :rows="2" placeholder="请输入补充说明"
...
<script>
  ...
  import {
    deleteFinanceBusinessExpenseMaster,
    reviewFinanceBusinessExpenseMaster,
    saveFinanceBusinessExpenseMaster,
    getMasterAndDetail,
    getNoPaymentAmount,
    getFinanceBusinessExpenseMasterDetail
  } from '@/api/bpmaManager/finance/cost/businessReimbursement.js'
  ...
  export default {
    ...
    activated() {
      this.$utilsBasic.loadAddAndDetailData(this, () => {
        ...
        if (this.$route.params.detail != 'detail') {
          await this.getFinanceBusinessExpenseMasterDetail(this.$route.params.detail)
        } else {
          this.initProjectFormData()
          this.costTypeList = []
        }
    ...
    methods: {
      ...
      getFinanceBusinessExpenseMasterDetail(id) {
        return getFinanceBusinessExpenseMasterDetail(id).then(res => {
          if (res.data) {
            this.formData = res.data
            getMasterAndDetail(this.$route.params.detail).then(res => {
              this.costTypeList = res.data.bpmaFinanceBusinessExpenseDetails || []
            })
          } else {
            this.initProjectFormData()
          }
      ...
      initFormData(lock) {
        ...
        if (lock) {
          this.initProjectFormData()
          this.$router.replace({
            path: `/bpmaManager/finance/cost/businessReimbursement/id/0/detail`,
            query: proData
          })
        } else {
          this.$router.replace({
            path: `/bpmaManager/finance/cost/businessReimbursement/id/0/${this.formData.id}`,
            query: proData
          })
        }
      },
      deleteData() {
        if (this.formData.id) {
          this.$confirm('亲，您是否确定删除当前数据，删除后数据不能恢复！', '提示', {
            ...
            deleteFinanceBusinessExpenseMaster(this.formData.id).then(res => {
              if (res.rel) {
      ...
      review() {
        this.$confirm(' 亲，您是否确定审核当前数据，审核后数据不能修改！', '提示', {
          ...
          let data = {
            bpmaFinanceBusinessExpenseMaster: this.formData,
            bpmaFinanceBusinessExpenseDetails: this.costTypeList
          }
          reviewFinanceBusinessExpenseMaster(data).then(res => {
            this.formData = res.data.bpmaFinanceBusinessExpenseMaster
            this.costTypeList = res.data.bpmaFinanceBusinessExpenseDetails
            if(res.data.bpmaFinanceBusinessExpenseMaster.processInstanceId) {
              return this.$store.dispatch('OpenSelectUserAndHandleAudit', {
                processId: res.data.bpmaFinanceBusinessExpenseMaster.processInstanceId,
                formData: res.data.bpmaFinanceBusinessExpenseMaster
              }).then(() => {
                this.$message.success(res.message);
              })
            } else {
      ...
      save(lock) {
        return new Promise((reslove, reject) => {
          ...
          this.$refs.form.validate(async valid => {
            ...
            res = await saveFinanceBusinessExpenseMaster(data)
