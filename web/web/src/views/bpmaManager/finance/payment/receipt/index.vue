      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增收票</el-button>
...
<script>
  ...
  import { getProjectList, getFinancePayBill } from '@/api/bpmaManager/finance/payment/receipt.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinancePayBill() {
        ...
        getFinancePayBill(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        ...
        if (lock) {
          this.$router.push({
            path: `/bpmaManager/finance/payment/receipt/id/0/detail`,
            query: this.proData
          })
        } else {
          this.$router.push({
            path: `/bpmaManager/finance/payment/receipt/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
