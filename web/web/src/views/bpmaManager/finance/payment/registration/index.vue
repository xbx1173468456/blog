      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增应付</el-button>
...
<script>
  ...
  import { getProjectList, getFinanceShouldPay } from '@/api/bpmaManager/finance/payment/registration.js'
  ...
  export default {
    ...
    methods: {
      ...
      getFinanceShouldPay() {
        ...
        getFinanceShouldPay(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getFinanceShouldPay()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增应收！')
        let { formData } = this
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/finance/payment/registration/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
