      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增跟踪</el-button>
...
<script>
  ...
  import { getProjectList, getScheduleTrackMaster } from '@/api/bpmaManager/schedule/duration/recording'
  ...
  export default {
    ...
    methods: {
      ...
      getScheduleTrackMaster() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getScheduleTrackMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
