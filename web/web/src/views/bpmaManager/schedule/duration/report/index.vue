      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增进度报告
              </el-button>
...
<script>
  ...
  import { getProjectList, getScheduleReportMaster } from '@/api/bpmaManager/schedule/duration/report'
  ...
  export default {
    ...
    methods: {
      ...
      getScheduleReportMaster() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getScheduleReportMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getScheduleReportMaster()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增进度报告！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/schedule/duration/report/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
