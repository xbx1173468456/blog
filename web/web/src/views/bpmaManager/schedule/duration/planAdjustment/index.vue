      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增计划调整
              </el-button>
...
<script>
  ...
  import { getProjectList, getSchedulePlanChangeMaster } from '@/api/bpmaManager/schedule/duration/planAdjustment'
  ...
  export default {
    ...
    methods: {
      ...
      getSchedulePlanChangeMaster() {
        this.tableShow = true
        if(!this.modelSearchFilter.proId) return
        getSchedulePlanChangeMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      contractChange(data) {
        ...
        this.getSchedulePlanChangeMaster()
      },
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增计划调整！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/schedule/duration/planAdjustment/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'changedBeginDate':
            return row.changedBeginDate ? row.changedBeginDate.split(' ')[0] : ''
          case 'changedExpiryDate':
            return row.changedExpiryDate ? row.changedExpiryDate.split(' ')[0] : ''
          default:
            return cellValue
        }
      },
