      ...
      <basic-split-pane class="bpmaManager-container-list" :default-percent='75' :min-percent='25'>
        <template slot="leftContainer">
          <div class="left-container">
            <basic-handler>
              <el-button @click="addClick(true)" icon="el-icon-plus" plain size="small" type="primary">新增计划编审
              </el-button>
...
<script>
  ...
  import { getProjectList, getSchedulePlanMaster } from '@/api/bpmaManager/schedule/duration/planEdit'
  ...
  export default {
    ...
    methods: {
      ...
      getSchedulePlanMaster() {
        ...
        getSchedulePlanMaster(this.modelSearchFilter).then(res => {
          this.formData = {}
      ...
      addClick(lock) {
        if (!this.proData.id) return this.$message.warning('当前无项目，无法新增计划编审！')
        ...
        if (lock) {
          ...
          this.$router.push({
            path: `/bpmaManager/schedule/duration/planEdit/id/0/${formData.id}`,
            query: this.proData
          })
        }
      },
      format(row, column, cellValue, index) {
        ...
        switch (column.property) {
          ...
          case 'contractBeginDate':
            return row.contractBeginDate ? row.contractBeginDate.split(' ')[0] : ''
          case 'contractExpiryDate':
            return row.contractExpiryDate ? row.contractExpiryDate.split(' ')[0] : ''
          case 'planBeginDate':
            return row.planBeginDate ? row.planBeginDate.split(' ')[0] : ''
          case 'planExpiryDate':
            return row.planExpiryDate ? row.planExpiryDate.split(' ')[0] : ''
          default:
            return cellValue
        }
      },
