<template>
  <basic-split-pane class="bpmaManager-container-list employment-container"
                    :default-percent='75'
                    :min-percent='25'>
        ...
        <div class="left-container">
          ...
          <div class="handler-container">
              <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                         @click="addClick('add')">新增用工申请</el-button>
          ...
          <pagination  :total="total" :page.sync="modelSearchFilter.page" :limit.sync="modelSearchFilter.limit" @pagination="getAllemployment"/>
        </div>
        </template>
        <template slot="rightContainer">
          <div class="right-container">
            <page-select-filter :selectData="selectData" :data="employmentData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
          </div>
        </template>
    ...
    methods: {
      ...
      addClick(param) {
        ...
        if (param === 'add') {
          ...
          this.$router.push({name: 'EmploymentDetail', params: {id: this.currentRow.id}})
        }
      },
      /**
       * 获取用工申请列表所有数据
      ...
      getAllemployment () {
        ...
        this.$store.dispatch('GetAllEmployment',this.modelSearchFilter).then( (res) => {
          if(res.status && res.status === 200){

            this.tableData = res.data.rows;
            this.total = res.data.total;
            console.log('获取用工申请列表所有数据',res)
          }else{
       ...
       * */
      getAllItem () {
        this.$store.dispatch('GetAllEmploymentItem',{page: 1, limit: 10000}).then((res) =>{

          if(res.status && res.status === 200){
            console.log('获取用工申请项目所有数据',res)

            //this.employmentData.data = res.data.rows;
            this.$set(this.employmentData,'data',res.data.rows)
            console.log('employmentData',this.employmentData.data)
          }else{
            this.$message({message: res.message,type: 'warning'})
          }
        }).catch(error => {
       ...
       * */
      getStage () {
        this.$store.dispatch('GetDictionaryDataByTypeFilterEmployment','SYS_PRO_STAGE')
          .then( (res) => {
            if(res.status && res.status === 200){
      ...
      format(row, column, cellValue, index) {
        switch(column.property) {
          case'applyDate':
            return dateFormat(row.applyDate)
          default:
            return cellValue
        }
      },
