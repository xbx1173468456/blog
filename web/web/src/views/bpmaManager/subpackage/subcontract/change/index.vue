<template>
  <basic-split-pane class="bpmaManager-container-list subContractChange-container"
                    :default-percent='75'
                    :min-percent='25'
  >
        ...
        <div class="left-container">
          ...
          <div class="handler-container">
              <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                         @click="addClick('add')">新增变更单</el-button>
          ...
          <pagination  :total="total" :page.sync="modelSearchFilter.page" :limit.sync="modelSearchFilter.limit" @pagination="getAllSubContractChangeData"/>
        </div>
        </template>
        <template slot="rightContainer">
          <div class="right-container">
            <page-select-filter :selectData="selectData" :data="subContractChangeData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
          </div>
        </template>
  </basic-split-pane>
</template>
...
<script>
  ...
  export default {
    ...
    activated(){
      this.getAllSubContractChangeData()
    },
    created() {
    ...
    methods: {
      ...
      addClick(param) {
        ...
        if (param === 'add') {
          ...
          this.$router.push({
            ...
            this.$message({message:'当前无项目，无法新增变更单!',type:'warning'})
          }
        } else {
          this.$router.push({name: 'ChangeDetail', params: {id: this.currentRow.id}})
        }
      },
      /**
       * 获取合同变更列表所有数据
       * */
      getAllSubContractChangeData () {
        // console.log('this.modelSearchFilter',this.modelSearchFilter)
        getAllSubContractChange(this.modelSearchFilter).then( (res) => {
          if(res.status && res.status === 200){

            this.tableData = res.data.rows;
            this.total = res.data.total;
            console.log('获取合同变更列表所有数据',res)
          }else{
       ...
       * */
      getAllItem () {
        getAllSubContractChangeItem({page: 1, limit: 10000}).then((res) =>{

          if(res.status && res.status === 200){
            console.log('获取合同变更项目所有数据',res)

            //this.subContractChangeData.data = res.data.rows;
            this.$set(this.subContractChangeData,'data',res.data.rows)
            console.log('subContractChangeData',this.subContractChangeData.data)
          }else{
            this.$message({message: res.message,type: 'warning'})
          }
        }).catch(error => {
      ...
      format(row, column, cellValue, index) {
        switch(column.property) {
          case'changeDate':
            return dateFormat(row.changeDate)
          case'price':
            return  this.$utilsBasic.toThousands(Number(row.price))
          default:
            return cellValue
        }
      },
