<template>
  <basic-split-pane class="bpmaManager-container-list visa-container"
                    :default-percent='75'
                    :min-percent='25'>
      ...
      <div class="left-container">
        <basic-handler>
          <el-button v-if="addBtnShowFlag" type="primary" plain icon="el-icon-plus" size="small"
                     @click="addClick('add')">新增签证</el-button>
        ...
        <basic-table ref="refVisaTableData"
                     :dataTable="visaTableData"
                     :columns="columns"
                     @current-change="handleCurrentChange"
                     :pagination="modelSearchFilter"
                     @pagination="getAllVisaData"
    ...
    <template slot="rightContainer">
      <div class="right-container" >
        <page-select-filter :selectData="selectData" :data="visaData" v-model="selectValue" @change="contractChange" @refreshData="getAllItem"></page-select-filter>
      </div>
    </template>
      ...
      methods:{
        contractChange(data) {
          ...
          if (!data) {
            ...
            return this.visaTableData.data = []
          }
        ...
        addClick(param) {
          ...
          if (param === 'add') {
            if(!this.$utils.isEmpty(this.proData)){
              ...
              this.$message({message:'当前无项目，无法新增签证单!',type:'warning'})
            }
          } else {
            this.$router.push({name: 'VisaDetail', params: {id: this.currentRow.id}})
          }
        },
        /**
         * 获取签证列表所有数据
        ...
        getAllVisaData () {
          ...
          getAllVisa(this.modelSearchFilter).then( (res) => {
            if(res.status && res.status === 200){

              this.visaTableData.data = res.data.rows;
              this.visaTableData.total = res.data.total;
              console.log('获取签证列表所有数据',res)
            }else{
         ...
         * */
        getAllItem () {
          getAllVisaItem({page: 1, limit: 10000}).then((res) =>{

            if(res.status && res.status === 200){
              console.log('获取签证项目所有数据',res)

              //this.subContractChangeData.data = res.data.rows;
              this.$set(this.visaData,'data',res.data.rows)
              console.log('visaData',this.visaData.data)
            }else{
              this.$message({message: res.message,type: 'warning'})
            }
          }).catch(error => {
         ...
         */
        clickSearchFilter() {
          this.$refs.refBasicSearch.searchFilter(this.$refs.refVisaTableData, this.modelSearchFilter, () => {
            this.getAllVisaData();
          });
        },
        /**
